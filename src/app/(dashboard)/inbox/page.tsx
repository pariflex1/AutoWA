...
/**
 * Sort conversations most-recent-first. Conversations without a
 * last_message_at (brand-new, no messages yet) go to the bottom so
 * they don't displace active threads. Called after every realtime
 * mutation that changes last_message_at to keep the list ordered
...
export default function InboxPage() {
              ...
              prev.map((c) =>
                c.id === conv.id
                  ? {
                      ...c,
                      ...conv,
                      // Realtime payloads never include the `contact` join,
                      // so spreading `conv` would overwrite the existing
                      // contact with undefined. Preserve the current value.
  ...
  const handleNewMessage = useCallback((msg: Message) => {
    ...
    // Optimistically update the conversation list preview so the
    // sent message text and timestamp appear immediately instead of
    // waiting for a realtime event (which may be delayed or lost).
    setConversations((prev) =>
      sortConversations(
        prev.map((c) =>
